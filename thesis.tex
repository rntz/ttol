...
% TODO: paragraph on why we use CAM variant
% - simple translation
% - but still an abstract machine (instruction sequence, state machine)
% - not particularly efficient, but this is not a design goal
...
handle our \ms{lib}, \ms{load}, and \ms{use} constructs. We do this in Figure
\ref{fig:camlib}.
...
% We extend our CAM to the \camlib{} by adding extending values $v$ to include
% (canonical) libraries, and replacing our environment $e$ with a pair $E =
% \<e,\sigma\>$ of a normal environment $e$ and a library substitution $\sigma$.
% Our state tuple thus becomes $\<c,\<e,\sigma\>\!,s\>$. Informally, $\sigma$
% represents locally bound library variables in the same sense that $e$ represents
% locally bound term variables.

% In order to efficiently implement hereditary
% substitution with the \camlib{}, however, we must use a variant of explicit
% substitutions, as will be explained in \ref{sec:explsubst}; so we call it a
% library \emph{substitution} rather than a library \emph{environment}. We must
% also extend the syntax of canonical and atomic libraries $M$, $R$ to support
% explicit substitution, renaming them $\M$, $\R$ respectively.
